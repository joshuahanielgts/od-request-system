...
import { Calendar, Clock, User, CheckCircle, XCircle, FileText, Home } from "lucide-react";
...
const ClassInChargeDashboard = () => {
  ...
  const fetchRequests = async (date?: string) => {
    try {
      let query = supabase
        .from('od_requests')
        .select('*')
        .eq('status', 'pending') // Class in charge only sees pending requests
  ...
  const handleRequestAction = async (requestId: string, action: "class_approved" | "rejected") => {
    try {
      setLoading(true);
      ...
      const message = action === "class_approved" 
        ? "OD request approved by Class In Charge" 
        : "OD request rejected by Class In Charge";

      toast({
        title: `Request ${action === "class_approved" ? "Approved" : "Rejected"}`,
        description: `${request?.student_name}'s request: ${message}`,
        variant: action === "class_approved" ? "default" : "destructive"
      });
  ...
  return (
    <Layout title="Class In Charge Dashboard">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-foreground">
              {selectedDate ? `Requests for ${new Date(selectedDate).toLocaleDateString()}` : `Pending Approvals`} ({requests.length})
            </h2>
            <p className="text-muted-foreground">Welcome to the Class In Charge Portal!</p>
          </div>
          ...
          <Card>
            <CardContent className="p-12 text-center">
              <div className="text-muted-foreground">
                ...
                <p>No pending OD requests for your approval at the moment.</p>
              </div>
            </CardContent>
          </Card>
        ) : (
              ...
              <>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-3 pt-4">
                    <Button
                      onClick={() => handleRequestAction(selectedRequest.id, "class_approved")}
